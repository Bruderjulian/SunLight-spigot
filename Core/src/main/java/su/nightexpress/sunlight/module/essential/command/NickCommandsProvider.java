...
public class NickCommandsProvider extends AbstractCommandProvider {
    ...
    private static final MessageLocale COMMAND_NICK_CHANGE_ERROR_REGEX = LangEntry.builder("Command.Nick.Error.Regex")
            .chatMessage(
    ...
    private boolean changeNick(CommandContext context, ParsedArguments arguments) {
        ...
        if (!player.hasPermission(NICK_BYPASS_LENGTH)) {
            if (raw.length() < this.settings.nickMinLength.get()) {
                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_SHORT, context.getSender(), replacer -> replacer
                        .with(SLPlaceholders.GENERIC_AMOUNT, () -> String.valueOf(this.settings.nickMinLength.get())));
                return false;
            }
            if (raw.length() > this.settings.nickMaxLength.get()) {
                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_LONG, context.getSender(), replacer -> replacer
                        .with(SLPlaceholders.GENERIC_AMOUNT, () -> String.valueOf(this.settings.nickMaxLength.get())));
                return false;
            }
        }

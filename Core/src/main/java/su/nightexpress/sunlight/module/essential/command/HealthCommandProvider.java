...
public class HealthCommandProvider extends AbstractCommandProvider {
    ...
    private static final MessageLocale MESSAGE_ADD_FEEDBACK = LangEntry.builder("Command.Health.Add.Target")
            .chatMessage(
                    ...
                    GRAY.wrap("You have " + GREEN.wrap("healed") + " " + WHITE.wrap(PLAYER_DISPLAY_NAME) + " for "
                            + GREEN.wrap(GENERIC_AMOUNT + "❤") + " (" + WHITE.wrap(GENERIC_OLD_VALUE + "❤")
    ...
    private static final MessageLocale MESSAGE_REMOVE_FEEDBACK = LangEntry.builder("Command.Health.Remove.Target")
            .chatMessage(
                    ...
                    GRAY.wrap("You have " + RED.wrap("damaged") + " " + WHITE.wrap(PLAYER_DISPLAY_NAME) + " for "
                            + RED.wrap(GENERIC_AMOUNT + "❤") + " (" + WHITE.wrap(GENERIC_OLD_VALUE + "❤")
    ...
    private static final MessageLocale MESSAGE_SET_FEEDBACK = LangEntry.builder("Command.Health.Set.Target")
            .chatMessage(
                    Sound.ENTITY_WITCH_DRINK,
                    GRAY.wrap("You have " + YELLOW.wrap("set") + " " + WHITE.wrap(PLAYER_DISPLAY_NAME) + "'s health to "
                            + YELLOW.wrap(GENERIC_AMOUNT + "❤") + " (" + WHITE.wrap(GENERIC_OLD_VALUE + "❤")
                            + DARK_GRAY.wrap(" → ") + RED.wrap(GENERIC_NEW_VALUE + "❤") + ")"));
    ...
    private static final MessageLocale MESSAGE_DEAD_FEEDBACK = LangEntry.builder("Command.Health.TargetDead.Feedback")
            .chatMessage(
        ...
        builder
                ...
                .withArguments(
                        Arguments.decimal(CommandArguments.AMOUNT, 0)
                                ...
                                .suggestions((reader, tabContext) -> IntStream.range(0, 21).boxed().map(String::valueOf)
                                        .toList()),
    ...
    private boolean changeHealth(CommandContext context, ParsedArguments arguments, ModifyMode mode) {
        return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager, target -> {
            if (target.isDead()) {
                context.send(context.getSender() == target ? MESSAGE_DEAD_NOTIFY : MESSAGE_DEAD_FEEDBACK,

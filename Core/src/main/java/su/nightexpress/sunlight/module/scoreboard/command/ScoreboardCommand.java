...
public class ScoreboardCommand extends AbstractCommandProvider {
    ...
    public ScoreboardCommand(SunLightPlugin plugin, ScoreboardModule module, UserManager userManager) {
        super(plugin);
        this.module = module;
    ...
    public void registerDefaults() {
        this.registerLiteral(COMMAND_TOGGLE, true, new String[] { "scoreboard", "board", "sb" },
                builder -> this.buildToggleCommand(builder, ToggleMode.TOGGLE));
        this.registerLiteral(COMMAND_ON, true, new String[] { "scoreboard-on", "board-on", "sb-on" },
                builder -> this.buildToggleCommand(builder, ToggleMode.ON));
        this.registerLiteral(COMMAND_OFF, true, new String[] { "scoreboard-off", "board-off", "sb-off" },
                builder -> this.buildToggleCommand(builder, ToggleMode.OFF));
    }

    private void buildToggleCommand(LiteralNodeBuilder builder, ToggleMode mode) {
        ...
        builder
                ...
                .withArguments(Arguments.playerName(CommandArguments.PLAYER)
                        .permission(SBPerms.COMMAND_SCOREBOARD_OTHERS).optional())
                .withFlags(CommandArguments.FLAG_SILENT)
                .executes((context, arguments) -> this.toggleScoreboard(context, arguments, mode));
    }

    private boolean toggleScoreboard(CommandContext context, ParsedArguments arguments, ToggleMode mode) {
        this.loadPlayerOrSenderWithDataAndRunInMainThread(context, arguments, this.module, this.userManager,
                (user, target) -> {
                    ...
                    if (context.getSender() != target) {
                        this.module.sendPrefixed(SBLang.COMMAND_SCOREBOARD_TARGET, context.getSender(),
                                builder -> builder

...
public class GamemodeCommandProvider extends AbstractCommandProvider {
        ...
        private static final MessageLocale MESSAGE_SET_FEEDBACK = LangEntry.builder("Command.GameMode.Target")
                        .chatMessage(
        ...
        public GamemodeCommandProvider(SunLightPlugin plugin, EssentialModule module, UserManager userManager) {
                super(plugin);
                this.module = module;
        ...
        public void registerDefaults() {
                this.registerLiteral(COMMAND_SURVIVAL, true, new String[] { "gms", "survival" },
                                builder -> this.builderType(builder, GameMode.SURVIVAL));
                this.registerLiteral(COMMAND_SPECTATOR, true, new String[] { "gmsp", "spectator" },
                                builder -> this.builderType(builder, GameMode.SPECTATOR));
                this.registerLiteral(COMMAND_ADVENTURE, true, new String[] { "gma", "adventure" },
                                builder -> this.builderType(builder, GameMode.ADVENTURE));
                this.registerLiteral(COMMAND_CREATIVE, true, new String[] { "gmc", "creative" },
                                builder -> this.builderType(builder, GameMode.CREATIVE));
        ...
        private void builderType(LiteralNodeBuilder builder, GameMode mode) {
                ...
                builder
                                .description(
                                                DESCRIPTION_TYPE.text().replace(SLPlaceholders.GENERIC_TYPE,
                                                                Lang.GAME_MODE.getLocalized(mode)))
                                .permission(permission)
                                .withArguments(Arguments.playerName(CommandArguments.PLAYER).permission(PERM_OTEHRS)
                                                .optional())
                                .withFlags(CommandArguments.FLAG_SILENT)
                                .executes((context, arguments) -> this.changeGameMode(context, arguments, mode));
        }

        private boolean changeGameMode(CommandContext context, ParsedArguments arguments, GameMode mode) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        if (target.isOnline()) {
                                                ...
                                                if (internals == null) {
                                                        this.module.sendPrefixed(Lang.ERROR_NO_INTERNALS_HANDLER,
                                                                        context.getSender());
                                                        return;
                                                }
                                        ...
                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                                                this.module.sendPrefixed(MESSAGE_SET_NOTIFY, target, builder -> builder
                                                                .with(SLPlaceholders.GENERIC_TYPE, () -> Lang.GAME_MODE
                                                                                .getLocalized(mode)));
                                        }
                                });
        }

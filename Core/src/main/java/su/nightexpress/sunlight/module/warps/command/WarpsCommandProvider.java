...
public class WarpsCommandProvider extends AbstractCommandProvider {
    ...
    public WarpsCommandProvider(SunLightPlugin plugin, WarpsModule module, UserManager userManager) {
        super(plugin);
        this.module = module;
    ...
    private ArgumentNodeBuilder<Warp> warpArgument(Predicate<Warp> predicate) {
    ...
    public void registerDefaults() {
        ...
        this.registerLiteral(COMMAND_JUMP, true, new String[] { "warp" }, builder -> builder
                ...
                .withArguments(
                        this.warpArgument(Warp::isActive).suggestions((reader, context) -> {
                            ...
                            return (player == null ? this.module.getWarps() : this.module.getAvailableWarps(player))
                                    .stream()
        ...
        this.registerRoot("Warps", true, new String[] { "warps" },
                ...
                builder -> builder.description(WarpsLang.COMMAND_WARPS_ROOT_DESC)
                        .permission(WarpsPerms.COMMAND_WARPS_ROOT));
    }

    private boolean createWarp(CommandContext context, ParsedArguments arguments) {
        Player player = context.getPlayerOrThrow();

        return this.module.create(player, arguments.getString(CommandArguments.NAME), false);
    }

    private boolean updateWarp(CommandContext context, ParsedArguments arguments) {
        Player player = context.getPlayerOrThrow();
    ...
    private boolean deleteWarp(CommandContext context, ParsedArguments arguments) {
        Warp warp = arguments.get(ARGUMENT_WARP, Warp.class);

        return this.module.removeWarp(context.getSender(), warp, false);
    }

    private boolean editWarp(CommandContext context, ParsedArguments arguments) {
        Warp warp = arguments.get(ARGUMENT_WARP, Warp.class);

        return this.module.openWarpSettings(context.getPlayerOrThrow(), warp);
    }

    private boolean openWarps(CommandContext context, ParsedArguments arguments) {
    ...
    private boolean jumpToWarp(CommandContext context, ParsedArguments arguments) {

...
public class AnnouncerSchema {
    ...
    public static final ConfigProperty<Boolean> RANDOM_ORDER = ConfigProperty.of(ConfigTypes.BOOLEAN, "RandomOrder",
            false,
            "Whether messages should go in random order. Otherwise the same as defined in the config.");

    public static final ConfigProperty<Set<String>> RANKS = ConfigProperty.of(ConfigTypes.STRING_SET_LOWER_CASE,
            "Ranks", Set.of(SLPlaceholders.WILDCARD),
            "Player ranks (permission group names) that are allowed to see this announcer messages.");

    public static final ConfigProperty<Map<String, List<String>>> TEXTS = ConfigProperty.of(
            ConfigTypes.forMapWithLowerKeys(ConfigTypes.STRING_LIST), "Texts", Collections.emptyMap(),
            "Announcer messages.",

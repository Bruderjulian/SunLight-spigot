...
public class TabSettings extends AbstractConfig {
    ...
    private final ConfigProperty<Long> playerListUpdateInterval = this.addProperty(ConfigTypes.LONG,
            "PlayerList.Update-Interval",
            20L,
    ...
    private final ConfigProperty<Long> playerListSortInterval = this.addProperty(ConfigTypes.LONG,
            "PlayerList.Sort-Interval",
            100L,
    ...
    private final ConfigProperty<Map<String, Integer>> playerListRankOrders = this.addProperty(
            ConfigTypes.forMapWithLowerKeys(ConfigTypes.INT),
            ...
            "For players who do not belong to any group (rank) listed here, the '%s' value is used if present; otherwise, it defaults to 0."
                    .formatted(DEFAULT),
            "",
            "[*] Requires %s OR %s with a compatible Permissions plugin for the feature to work."
                    .formatted(PermissionPlugins.LUCK_PERMS, PermissionPlugins.VAULT));

    private final ConfigProperty<Map<String, TabLayoutFormat>> playerListLayoutFormats = this.addProperty(
            ConfigTypes.forMapWithLowerKeys(TAB_LAYOUT_FORMAT_CONFIG_TYPE),
            ...
            "├── Worlds:",
            "│     -> List of worlds, where this format is available. Add '%s' to allow all worlds."
                    .formatted(SLPlaceholders.WILDCARD),
            "├── Groups:",
            "│     -> List of ranks (permission groups) to which this format is available. Add '%s' to allow all ranks."
                    .formatted(SLPlaceholders.WILDCARD),
            "└── Header + Footer:",
            ...
            "          - %s - include dynamic text (animation) from the '%s'."
                    .formatted(SLPlaceholders.ANIMATION.apply("[name]"), TabFiles.FILE_ANIMATIONS),
            ...
            "",
            "[*] Requires %s OR %s with a compatible Permissions plugin for the feature to work properly."
                    .formatted(PermissionPlugins.LUCK_PERMS, PermissionPlugins.VAULT));

    private final ConfigProperty<Map<String, TabNameFormat>> playerListNameFormats = this.addProperty(
            ConfigTypes.forMapWithLowerKeys(TAB_NAME_FORMAT_CONFIG_TYPE),

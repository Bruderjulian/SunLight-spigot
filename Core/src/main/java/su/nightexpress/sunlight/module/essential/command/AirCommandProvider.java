...
public class AirCommandProvider extends AbstractCommandProvider {
        ...
        private static final TextLocale DESCRIPTION_ROOT = LangEntry.builder("Command.Air.Root.Desc")
                        .text("Add commands.");
        private static final TextLocale DESCRIPTION_ADD = LangEntry.builder("Command.Air.Add.Desc")
                        .text("Add air ticks.");
        private static final TextLocale DESCRIPTION_SET = LangEntry.builder("Command.Air.Set.Desc")
                        .text("Set air ticks.");
        private static final TextLocale DESCRIPTION_REMOVE = LangEntry.builder("Command.Air.Remove.Desc")
                        .text("Remove air ticks.");

        private static final MessageLocale ADD_TARGET = LangEntry.builder("Command.Air.Give.Target").chatMessage(
                        GRAY.wrap("You have added " + GREEN.wrap(GENERIC_AMOUNT) + " air ticks to "
                                        + SOFT_YELLOW.wrap(PLAYER_DISPLAY_NAME) + ". New air ticks: "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT)
                                        + "/" + SOFT_YELLOW.wrap(GENERIC_MAX) + "."));

        private static final MessageLocale REMOVE_TARGET = LangEntry.builder("Command.Air.Take.Target").chatMessage(
                        GRAY.wrap("You have removed " + RED.wrap(GENERIC_AMOUNT) + " air ticks from "
                                        + SOFT_YELLOW.wrap(PLAYER_DISPLAY_NAME) + ". New air ticks: "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT)
                                        + "/" + SOFT_YELLOW.wrap(GENERIC_MAX) + "."));

        private static final MessageLocale SET_TARGET = LangEntry.builder("Command.Air.Set.Target").chatMessage(
                        GRAY.wrap("You have set " + YELLOW.wrap(GENERIC_AMOUNT) + " air ticks for "
                                        + SOFT_YELLOW.wrap(PLAYER_DISPLAY_NAME) + ". New air ticks: "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT)
                                        + "/" + SOFT_YELLOW.wrap(GENERIC_MAX) + "."));

        private static final MessageLocale ADD_NOTIFY = LangEntry.builder("Command.Air.Give.Notify").chatMessage(
                        GRAY.wrap("You got " + SOFT_YELLOW.wrap(GENERIC_AMOUNT) + " air ticks: "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT)
                                        + "/" + SOFT_YELLOW.wrap(GENERIC_MAX) + "."));

        private static final MessageLocale REMOVE_NOTIFY = LangEntry.builder("Command.Air.Take.Notify").chatMessage(
                        GRAY.wrap("You lost " + SOFT_YELLOW.wrap(GENERIC_AMOUNT) + " air ticks: "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT) + "/" + SOFT_YELLOW.wrap(GENERIC_MAX)
                                        + "."));

        private static final MessageLocale SET_NOTIFY = LangEntry.builder("Command.Air.Set.Notify").chatMessage(
                        GRAY.wrap("Your air ticks set to " + SOFT_YELLOW.wrap(GENERIC_AMOUNT) + ": "
                                        + SOFT_YELLOW.wrap(GENERIC_CURRENT) + "/" + SOFT_YELLOW.wrap(GENERIC_MAX)
                                        + "."));
        ...
        public AirCommandProvider(SunLightPlugin plugin, EssentialModule module, UserManager userManager) {
                super(plugin);
                this.module = module;
        ...
        public void registerDefaults() {
                this.registerLiteral(COMMAND_ADD, false, new String[] { "addair" },
                                builder -> this.buildExecutor(builder, ModifyMode.ADD));
                this.registerLiteral(COMMAND_SET, false, new String[] { "setair" },
                                builder -> this.buildExecutor(builder, ModifyMode.SET));
                this.registerLiteral(COMMAND_REMOVE, false, new String[] { "removeair" },
                                builder -> this.buildExecutor(builder, ModifyMode.REMOVE));
        ...
        private void buildExecutor(LiteralNodeBuilder builder, ModifyMode mode) {
                ...
                builder
                                ...
                                .withArguments(
                                                Arguments.integer(CommandArguments.AMOUNT, 0)
                                                                .localized(CoreLang.COMMAND_ARGUMENT_NAME_AMOUNT)
                                                                .suggestions((reader, context) -> Lists.newList("100",
                                                                                "200", "300", "400", "500")))
                                .withArguments(Arguments.playerName(CommandArguments.PLAYER)
                                                .permission(PERMISSION_OTHERS))
                                .withFlags(CommandArguments.FLAG_SILENT)
                                .executes((context, arguments) -> this.executeMode(context, arguments, mode));
        }

        private boolean executeMode(CommandContext context, ParsedArguments arguments, ModifyMode mode) {
                ...
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        target.setRemainingAir((int) Math.max(0,
                                                        mode.modify(target.getRemainingAir(), amount)));
                                        ...
                                        if (target != context.getSender()) {
                                                this.module.sendPrefixed(message, context.getSender(),
                                                                replacer -> replacer
                                                                                .with(SLPlaceholders.GENERIC_CURRENT,
                                                                                                () -> String.valueOf(
                                                                                                                target.getRemainingAir()))
                                                                                .with(SLPlaceholders.GENERIC_MAX,
                                                                                                () -> String.valueOf(

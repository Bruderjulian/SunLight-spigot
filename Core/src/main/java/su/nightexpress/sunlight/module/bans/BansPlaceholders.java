...
public class BansPlaceholders extends SLPlaceholders {
    ...
    private static final TypedPlaceholder<AbstractPunishment> PUNISHMENT = TypedPlaceholder
            ...
            .with(PUNISHMENT_DURATION,
                    punishment -> TimeFormats.formatAmount(punishment.getDuration(), TimeFormatType.LITERAL))
            .with(PUNISHMENT_EXPIRES_IN,
                    punishment -> !punishment.isPermanent()
                            ? TimeFormats.formatDuration(punishment.getExpirationDate(), TimeFormatType.LITERAL)
            ...
            .with(PUNISHMENT_EXPIRATION_DATE,
                    punishment -> !punishment.isPermanent() ? TimeFormats.formatDateTime(punishment.getExpirationDate())
                            : CoreLang.OTHER_NEVER.text())
            .build();

...
public class BansSettings extends AbstractConfig {
    ...
    public final ConfigProperty<Map<String, Integer>> rankPriorities = this.addProperty(
            ConfigTypes.forMapWithLowerKeys(ConfigTypes.INT), "Priority.Ranks",
            Map.of(
                    "moderator", 50,
                    "admin", 100,
                    "owner", 999),
    ...
    public final ConfigProperty<Map<String, Map<PunishmentType, BanTime>>> durationLimitsRanks = this.addProperty(
            ConfigTypes.forMap(LowerCase.INTERNAL::apply, s -> s,
                    ConfigTypes.forMap(string -> Enums.get(string, PunishmentType.class), Enum::name,
                            DURATION_LIMIT_CONFIG_TYPE)),
            ...
            "[*] Requires %s with a compatible permissions plugin OR %s to work.".formatted(PermissionPlugins.VAULT,
                    PermissionPlugins.LUCK_PERMS));

    public final ConfigProperty<Boolean> durationLimitsForPardon = this.addProperty(ConfigTypes.BOOLEAN,
            "Time.Duration-Limits.Include-For-Pardon",
            true,
    ...
    public final ConfigProperty<Set<String>> immunityList = this.addProperty(ConfigTypes.STRING_SET_LOWER_CASE,
            "Security.Immunity-List",
    ...
    public final ConfigProperty<List<String>> disconnectScreenKick = this.addProperty(ConfigTypes.STRING_LIST,
            "Disconnect-Screen.Kick",
            List.of(
    ...
    public final ConfigProperty<List<String>> disconnectScreenPermaBan = this.addProperty(ConfigTypes.STRING_LIST,
            "Disconnect-Screen.Permanent-Ban",
            Lists.newList(
                    SPRITE.apply("items", "item/barrier") + RED.and(BOLD).wrap(" YOU ARE PERMANENTLY BANNED ")
                            + SPRITE.apply("items", "item/barrier"),
                    "",
                    WHITE.wrap("You " + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
                            + " have been permanently banned " + YELLOW.wrap(PUNISHMENT_CREATION_DATE)
                            + " with the following reason: " + SOFT_RED.wrap("\"" + PUNISHMENT_REASON + "\"") + "."),
                    "",
                    WHITE.wrap("If you believe this happened by mistake, you can appeal the ban here: "
                            + GREEN.wrap("< INSERT YOUR URL >"))),
            "Text to display in disconnect window for permanently banned players.");

    public final ConfigProperty<List<String>> disconnectScreenTempBan = this.addProperty(ConfigTypes.STRING_LIST,
            "Disconnect-Screen.Temp-Ban",
            Lists.newList(
                    SPRITE.apply("items", "item/barrier") + RED.and(BOLD).wrap(" YOU ARE TEMPORARY BANNED ")
                            + SPRITE.apply("items", "item/barrier"),
                    "",
                    WHITE.wrap("You " + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
                            + " have been temporary banned " + YELLOW.wrap(PUNISHMENT_CREATION_DATE)
                            + " with the following reason: " + SOFT_RED.wrap("\"" + PUNISHMENT_REASON + "\"") + "."),
                    "",
                    WHITE.wrap("You will be unbanned " + GOLD.wrap(PUNISHMENT_EXPIRATION_DATE)
                            + GRAY.wrap(" (in " + PUNISHMENT_EXPIRES_IN + ")")),
                    "",
                    WHITE.wrap("If you believe this happened by mistake, you can appeal the ban here: "
                            + GREEN.wrap("< INSERT YOUR URL >"))),
            "Text to display in disconnect window for temporary banned players.");

    public final ConfigProperty<List<String>> disconnectScreenIpBan = this.addProperty(ConfigTypes.STRING_LIST,
            "Disconnect-Screen.IP-Ban",
            List.of(
                    SPRITE.apply("items", "item/barrier") + RED.and(BOLD).wrap(" YOUR IP ADDRESS IS BANNED ")
                            + SPRITE.apply("items", "item/barrier"),
                    "",
                    WHITE.wrap("Your IP address " + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
                            + " has been banned " + YELLOW.wrap(PUNISHMENT_CREATION_DATE)
                            + " with the following reason: " + SOFT_RED.wrap("\"" + PUNISHMENT_REASON + "\"") + "."),
                    "",
                    WHITE.wrap("If you believe this happened by mistake, you can appeal the ban here: "
                            + GREEN.wrap("< INSERT YOUR URL >"))),
    ...
    public final ConfigProperty<Boolean> altDetectorEnabled = this.addProperty(ConfigTypes.BOOLEAN,
            "AltDetector.Enabled",
            true,
            "Controls whether Alt Detector feature is enabled.");

    public final ConfigProperty<Boolean> muteBlockCommandsEnabled = this.addProperty(ConfigTypes.BOOLEAN,
            "Mute.Block-Commands.Enabled",
            true,
            "Controls whether muted players will be restricted from using certain commands.");

    public final ConfigProperty<Set<String>> muteBlockCommandsList = this.addProperty(ConfigTypes.STRING_SET,
            "Mute.Block-Commands.List",
            Set.of("me", "broadcast"),
            "Commands to block for muted players (without the '/' slash).");

    public final ConfigProperty<Integer> warnMaxAmountToReset = this.addProperty(ConfigTypes.INT,
            "Warn.Max-Amount-To-Reset",
            5,

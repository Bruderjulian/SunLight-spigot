...
public class KitsPlaceholders {
    ...
    public static final TypedPlaceholder<Kit> KIT = TypedPlaceholder.builder(Kit.class)
            ...
            .with(KIT_COOLDOWN,
                    kit -> kit.hasCooldown()
                            ? (kit.isOneTimed() ? CoreLang.OTHER_ONE_TIMED.text()
                                    : TimeFormats.formatAmount(kit.definition().getCooldown() * 1000L,
                                            TimeFormatType.LITERAL))
                            : CoreLang.STATE_YES_NO.get(false))
            .with(KIT_COST, kit -> EconomyBridge.economyCurrency().filter(currency -> kit.hasCost())
                    .map(currency -> currency.format(kit.definition().getCost())).orElse(Lang.OTHER_FREE.text()))
            .build();
}

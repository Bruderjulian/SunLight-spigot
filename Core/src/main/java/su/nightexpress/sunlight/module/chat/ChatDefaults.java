...
public class ChatDefaults {
    ...
    public static Map<String, FormatComponent> getDefaultFormatComponents() {
        ...
        map.put("player_info", new FormatComponent(
                SHOW_TEXT
                        .with(GRAY.wrap("Player: " + LIGHT_PURPLE.wrap(PLAYER_NAME) + BR + "Nickname: "
                                + LIGHT_PURPLE.wrap(PLAYER_DISPLAY_NAME) + BR + BR + GRAY.wrap("(Click to message)")))
                        .wrap(SUGGEST_COMMAND.with("/" + DEFAULT_PM_ALIAS + " " + PLAYER_NAME + " ")
        ...
        map.put("rank_member", new FormatComponent(
                SHOW_TEXT.with(GRAY.wrap("Type " + SOFT_GREEN.wrap("/ranks") + " to view all special ranks."))
    ...
    public static Map<SpyType, String> getDefaultSpyFormat() {
        ...
        map.put(SpyType.COMMAND, SOFT_RED.wrap("[CommandSpy]") + " " + GRAY.wrap(PLAYER_NAME + " executed a command:")
                + " " + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE));
        map.put(SpyType.CHAT,
                SOFT_RED.wrap("[ChatSpy]") + " " + GRAY.wrap("[" + CHANNEL_NAME + "]" + " " + PLAYER_NAME + ":") + " "
                        + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE));
        return map;
    }
}

...
public class CommandArguments {
    ...
    public static ArgumentNodeBuilder<EquipmentSlot> slot(String name) {
        return Commands.argument(name, (context, string) -> Enums.parse(string, EquipmentSlot.class)
                ...
                .suggestions((reader, context) -> Arrays.stream(EntityUtil.EQUIPMENT_SLOTS).map(Enum::name)
        ...
        return Commands
                .argument(name,
                        (context, str) -> Enums.parse(str, clazz).orElseThrow(
                                () -> CommandSyntaxException.custom(Lang.ERROR_COMMAND_INVALID_TYPE_ARGUMENT)))
    ...
    public static ArgumentNodeBuilder<EntityType> entityType(Predicate<EntityType> predicate) {
        return Commands.argument(TYPE, (context, string) -> Optional.ofNullable(BukkitThing.getEntityType(string))
                ...
                .suggestions((reader, tabContext) -> BukkitThing.getAll(RegistryType.ENTITY_TYPE).stream()
    ...
    public static ArgumentNodeBuilder<PotionEffectType> effect(String name, Predicate<PotionEffectType> predicate) {
        return Commands.argument(name, (context, string) -> Optional.ofNullable(BukkitThing.getEffectType(string))
                ...
                .localized(Lang.COMMAND_ARGUMENT_NAME_TYPE.text())
                .suggestions((reader, tabContext) -> BukkitThing.getAll(RegistryType.MOB_EFFECT).stream()

...
public class FireTicksCommandsProvider extends AbstractCommandProvider {
        ...
        private static final MessageLocale MESSAGE_EXTINGUISH_FEEDBACK = LangEntry
                        .builder("Command.Extinguish.Feedback")
                        .chatMessage(
        ...
        private static final MessageLocale MESSAGE_IGNITE_FEEDBACK = LangEntry.builder("Command.Ignite.Feedback")
                        .chatMessage(
                                        ...
                                        GRAY.wrap("You have ignited " + WHITE.wrap(PLAYER_DISPLAY_NAME) + " for "
                                                        + SOFT_YELLOW.wrap(GENERIC_TIME)));

        private static final MessageLocale MESSAGE_IGNITE_NOTIFY = LangEntry.builder("Command.Ignite.Notify")
                        .chatMessage(
                                        Sound.ENTITY_PLAYER_HURT_ON_FIRE,
        ...
        public FireTicksCommandsProvider(SunLightPlugin plugin, EssentialModule module, UserManager userManager) {
                super(plugin);
                this.module = module;
        ...
        public void registerDefaults() {
                this.registerLiteral(COMMAND_IGNITE, true, new String[] { "ignite" }, builder -> builder
                                ...
                                .withArguments(
                                                Arguments.integer(CommandArguments.TIME, 1, 86400)
                                                                ...
                                                                .suggestions((reader, context) -> Lists.newList("5",
                                                                                "10", "30", "60")),
                                                Arguments.playerName(CommandArguments.PLAYER)
                                                                .permission(PERMISSION_IGNITE_OTHERS).optional())
                ...
                this.registerLiteral(COMMAND_EXTINGUISH, true, new String[] { "extinguish" }, builder -> builder
                                ...
                                .withArguments(
                                                Arguments.playerName(CommandArguments.PLAYER)
                                                                .permission(PERMISSION_EXTINGUISH_OTHERS)
                                                                .optional())
                                .withFlags(CommandArguments.FLAG_SILENT)
                                .executes(this::extinguish));
        }

        private boolean ignite(CommandContext context, ParsedArguments arguments) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        ...
                                        if (context.getSender() != target) {
                                                this.module.sendPrefixed(MESSAGE_IGNITE_FEEDBACK, context.getSender(),
                                                                builder -> builder
                                        ...
                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                                                this.module.sendPrefixed(MESSAGE_IGNITE_NOTIFY, target,
                                                                builder -> builder
                                                                                .with(SLPlaceholders.GENERIC_TIME,
                                                                                                () -> TimeFormats
                                                                                                                .toLiteral(seconds
        ...
        private boolean extinguish(CommandContext context, ParsedArguments arguments) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        target.setFireTicks(0);

                                        if (context.getSender() != target) {
                                                this.module.sendPrefixed(MESSAGE_EXTINGUISH_FEEDBACK,
                                                                context.getSender(), builder -> builder.with(

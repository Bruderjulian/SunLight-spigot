package su.nightexpress.sunlight.moduleImpl.warps.dialog.impl;
    ...
    public WrappedDialog create(Player player, Warp warp) {
        return Dialogs.builder()
                .base(DialogBases.builder(TITLE)
                        ...
                        .inputs(
                                DialogInputs.text(JSON_PAGE, INPUT_PAGE).maxLength(2)
                                        .initial(String.valueOf(warp.getMenuPage()))
                                        .build(),
                                DialogInputs.text(JSON_SLOTS, INPUT_SLOTS).maxLength(30)
                                        .initial(ArrayUtil.arrayToString(warp
                                                .getMenuSlots()))
                                        .build())
                        .build())
                ...
                .handleResponse(DialogActions.OK, (viewer, identifier, nbtHolder) -> {
                    ...
                    int[] slots = nbtHolder.getText(JSON_SLOTS).map(ArrayUtil::parseIntArray)
                            .orElse(warp.getMenuSlots());

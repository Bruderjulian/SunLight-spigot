...
public class BansSettings extends AbstractConfig {
        ...
        public final ConfigProperty<Map<String, Map<PunishmentType, BanTime>>> durationLimitsRanks = this.addProperty(
                        ConfigTypes.forMap(LowerCase.INTERNAL::apply, s -> s,
                                        ConfigTypes.forMap(string -> Enums.get(string, PunishmentType.class),
                                                        Enum::name,
        ...
        public final ConfigProperty<List<String>> disconnectScreenPermaBan = this.addProperty(ConfigTypes.STRING_LIST,
                        ...
                        Lists.newList(
                                        SPRITE.apply("items", "item/barrier")
                                                        + RED.and(BOLD).wrap(" YOU ARE PERMANENTLY BANNED ")
                                                        + SPRITE.apply("items", "item/barrier"),
                                        "",
                                        WHITE.wrap("You " + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
                                                        + " have been permanently banned "
                                                        + YELLOW.wrap(PUNISHMENT_CREATION_DATE)
                                                        + " with the following reason: "
                                                        + SOFT_RED.wrap("\"" + PUNISHMENT_REASON + "\"") + "."),
                                        "",
        ...
        public final ConfigProperty<List<String>> disconnectScreenTempBan = this.addProperty(ConfigTypes.STRING_LIST,
                        ...
                        Lists.newList(
                                        SPRITE.apply("items", "item/barrier")
                                                        + RED.and(BOLD).wrap(" YOU ARE TEMPORARY BANNED ")
                                                        + SPRITE.apply("items", "item/barrier"),
                                        "",
                                        WHITE.wrap("You " + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
                                                        + " have been temporary banned "
                                                        + YELLOW.wrap(PUNISHMENT_CREATION_DATE)
                                                        + " with the following reason: "
                                                        + SOFT_RED.wrap("\"" + PUNISHMENT_REASON + "\"") + "."),
                                        "",
        ...
        public final ConfigProperty<List<String>> disconnectScreenIpBan = this.addProperty(ConfigTypes.STRING_LIST,
                        ...
                        List.of(
                                        SPRITE.apply("items", "item/barrier")
                                                        + RED.and(BOLD).wrap(" YOUR IP ADDRESS IS BANNED ")
                                                        + SPRITE.apply("items", "item/barrier"),
                                        "",
                                        WHITE.wrap("Your IP address "
                                                        + GRAY.wrap("(" + YELLOW.wrap(PUNISHMENT_TARGET) + ")")
        ...
        public final ConfigProperty<Map<Integer, List<String>>> warnAutoCommands = this.addProperty(
                        ConfigTypes.forMap(key -> Numbers.parseInteger(key).orElse(null), String::valueOf,
                        ...
                        "[*] Placeholders: '%s', '%s'".formatted(PLAYER_NAME,
                                        "Punishment (https://nightexpressdev.com/)"));
        ...
        private static Map<String, Map<PunishmentType, BanTime>> getDefaultDurationLimits() {
                ...
                Map<PunishmentType, BanTime> durationMap1 = map.computeIfAbsent("moderator",
                                k -> new LinkedHashMap<>());

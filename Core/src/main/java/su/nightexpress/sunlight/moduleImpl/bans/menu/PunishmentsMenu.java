...
public class PunishmentsMenu extends AbstractObjectMenu<PunishmentsMenu.Data> implements LangContainer {
    ...
    private static final EnumLocale<PunishmentType> TYPE_LOCALE = LangEntry.builder("Bans.UI.List.TypeName")
    ...
    public record Data(PunishmentType type, SortMode sortMode, boolean showExpired) {
    }
    ...
    public boolean show(Player player, PunishmentType type) {
                ...
                .defaultState(ItemState.defaultBuilder()
                        ...
                        .displayModifier((context, item) -> {
                            item.replace(builder -> builder.with(SLPlaceholders.GENERIC_STATE,
                                    () -> su.nightexpress.nightcore.core.config.CoreLang.STATE_ENABLED_DISALBED
                                            .get(this.getObject(context).showExpired)));
                        })
                        .action(context -> {
                            Data data = this.getObject(context);
    ...
    protected void onLoad(FileConfig config) {
        this.playerIcon = ConfigProperty
                .of(ConfigTypes.NIGHT_ITEM, "Punishment.Icon.Player", NightItem.fromType(Material.PLAYER_HEAD))
                .resolveWithDefaults(config);
        this.inetIcon = ConfigProperty
                .of(ConfigTypes.NIGHT_ITEM, "Punishment.Icon.Inet", NightItem.fromType(Material.COMPARATOR))
                .resolveWithDefaults(config);
        ...
        this.populator = ItemPopulator.builder(AbstractPunishment.class)
                ...
                .itemProvider((context, punishment) -> {
                    ...
                    if (punishment instanceof PlayerPunishment playerPunishment) {
                        icon = this.playerIcon.copy().setPlayerProfile(PlayerProfiles
                                .createProfile(playerPunishment.getPlayerId(), playerPunishment.getPlayerName()));
                    } else {
                    ...
                    if (event.getClick() == ClickType.DROP) {
                        ...
                        if (punishment instanceof PlayerPunishment playerPunishment) {
                            this.module.getPunishmentRepository(data.type)
                                    .updatePlayerPunishmentReferences(playerPunishment);
                        } else if (punishment instanceof InetPunishment inetPunishment) {
                            this.module.getPunishmentRepository(data.type)
                                    .updateInetPunishmentReferences(inetPunishment);
                        }
                    } else
                        return;

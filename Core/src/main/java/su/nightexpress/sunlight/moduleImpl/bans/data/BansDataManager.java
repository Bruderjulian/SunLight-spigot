package su.nightexpress.sunlight.moduleImpl.bans.data;
...
public class BansDataManager {
    ...
    public BansDataManager(SunLightPlugin plugin, DataHandler dataHandler, BansModule module) {
        // this.plugin = plugin;
    ...
    private void syncPlayerPunishments(ResultSet resultSet) {
        try {
    ...
    private void syncInetPunishments(ResultSet resultSet) {
        try {
    ...
    public void purgeOldEntries() {
        ...
        for (Table table : tables) {
            ...
            this.dataHandler.delete(table, Wheres.where(COLUMN_EXPIRE_DATE, Operator.SMALLER, o -> deadlineMs)
                    .and(COLUMN_EXPIRE_DATE, Operator.GREATER, o -> 0));
        }
    }
    ...
    public void insertPunishment(InetPunishment bannedIp) {
        this.dataHandler.insert(this.tableBannedIPs, BansQueries.INSERT_IP, bannedIp);
    }

    public void insertPunishment(PlayerPunishment bannedPlayer) {
        this.dataHandler.insert(this.tableBannedPlayers, BansQueries.INSERT_PLAYER, bannedPlayer);
    }

    public void updateInetPunishments(Collection<InetPunishment> punishments) {
        if (punishments.isEmpty())
            return;

        this.dataHandler.update(this.tableBannedIPs, BansQueries.UPDATE_IP, punishments,
                Wheres.whereUUID(COLUMN_BAN_ID, AbstractPunishment::getId));
    }

    public void updatePlayerPunishments(Collection<PlayerPunishment> punishments) {
        if (punishments.isEmpty())
            return;

        this.dataHandler.update(this.tableBannedPlayers, BansQueries.UPDATE_PLAYER, punishments,
                Wheres.whereUUID(COLUMN_BAN_ID, AbstractPunishment::getId));
    }

    public void deleteInetPunishment(InetPunishment inetPunishment) {
        this.dataHandler.delete(this.tableBannedIPs, inetPunishment,
                Wheres.whereUUID(COLUMN_BAN_ID, AbstractPunishment::getId));
    }

    public void deletePlayerPunishment(PlayerPunishment playerPunishment) {
        this.dataHandler.delete(this.tableBannedPlayers, playerPunishment,
                Wheres.whereUUID(COLUMN_BAN_ID, AbstractPunishment::getId));
    }
}

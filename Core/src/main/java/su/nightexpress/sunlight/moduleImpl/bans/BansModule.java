package su.nightexpress.sunlight.moduleImpl.bans;
...
public class BansModule extends Module {
    ...
    private void loadData() {
        ...
        this.plugin.runTaskAsync(task -> {
            ...
            this.plugin.runTask(this::kickBanned); // Kick banned players in case when there are new database ban
                                                   // entries for online players.
        });
    }
    ...
    public PunishmentReason getReasonById(String id) {
    ...
    public PunishmentRepository getPunishmentRepository(AbstractPunishment punishment) {
        return this.getPunishmentRepository(punishment.getType());
    }

    public PunishmentRepository getPunishmentRepository(PunishmentType type) {
        return switch (type) {
    ...
    public Set<PlayerPunishment> getPlayerPunishments(PunishmentType type) {
        return this.getPunishmentRepository(type).getPlayerPunishments();
    }

    public Set<PlayerPunishment> getPlayerPunishments(UUID playerId, PunishmentType type) {
    ...
    public PlayerPunishment getActivePlayerPunishment(UUID playerId, PunishmentType type) {
        return this.getPunishmentRepository(type).getActivePlayerPunishment(playerId);
    }

    public Set<PlayerPunishment> getActivePlayerPunishments(UUID playerId, PunishmentType type) {
    ...
    public boolean hasActivePunishment(UUID playerId, PunishmentType type) {
        return !this.getActivePlayerPunishments(playerId, type).isEmpty();
    }

    public boolean hasActivePunishment(InetAddress address, PunishmentType type) {
        return !this.getActiveInetPunishments(address).isEmpty();
    }

    public boolean openPunishments(Player player, PunishmentType type) {
        return this.punishmentsMenu.show(player, type);
    }

    public boolean openHistory(Player player, UserInfo userInfo, PunishmentType type) {
        return this.historyMenu.show(player, userInfo, type);
    }

    private void addPlayerPunishment(PlayerPunishment punishment) {
        this.getPunishmentRepository(punishment).addPlayerPunishment(punishment);
        this.plugin.runTaskAsync(task -> this.dataManager.insertPunishment(punishment));
    }

    public void deletePlayerPunishment(PlayerPunishment punishment) {
    ...
    private void addInetPunishment(InetPunishment punishment) {
        this.getPunishmentRepository(punishment).addInetPunishment(punishment);
        this.plugin.runTaskAsync(task -> this.dataManager.insertPunishment(punishment));
    }

    public void deleteInetPunishment(InetPunishment punishment) {
    ...
    public boolean hasImmunity(InetAddress address) {
    ...
    public BanTime getGreatestDuration(CommandSender sender, PunishmentType type) {
        if (!(sender instanceof Player player))
            return BanTime.permanent(); // No limits for Console.
    ...
    public int getRankPriority(CommandSender sender) {
    ...
    private CompletableFuture<Integer> getRankPriority(UserInfo profile) {
        return Players.getInheritanceGroups(profile.id()).thenApply(this::getRankPriority);
    }

    private int getRankPriority(Set<String> ranks) {
    ...
    public void handleLoginEvent(AsyncPlayerPreLoginEvent event) {
    ...
    public void handleCommandEvent(PlayerCommandPreprocessEvent event) {
        if (!this.settings.muteBlockCommandsEnabled.get())
            return;
    ...
    public void handleChatEvent(UniversalChatEvent event) {
    ...
    public CompletableFuture<Boolean> punishPlayer(CommandSender sender, UserInfo victim,
    ...
    private boolean punishPlayer(CommandSender sender, UserInfo victim, int victimPriority,
            PunishmentContext context) {
        if (!this.dataLoaded) {
            this.sendPrefixed(BansLang.ERROR_DATA_NOT_LOADED_ADMIN_FEEDBACK, sender);
            return false;
        ...
        if (type == PunishmentType.WARN) {
            ...
            if (commands != null && !commands.isEmpty()) {
                ...
                // Apply placeholders per line to prevent custom commands with line breakers be
                // splitted by PlaceholderContext.
    ...
    public boolean banInet(CommandSender sender, InetAddress address,
            PunishmentReason reason, BanTime banTime, boolean silent) {
        if (!this.dataLoaded) {
            this.sendPrefixed(BansLang.ERROR_DATA_NOT_LOADED_ADMIN_FEEDBACK, sender);
            return false;
        }
        ...
        PunishmentData data = new PunishmentData(banId, PunishmentType.BAN, textReason, admin, duration, createDate,
                expireDate);
    ...
    private void notifyPunishment(AbstractPunishment punishment,
            CommandSender sender,
            MessageLocale feedbackLocale,
            MessageLocale broadcastLocale,
    ...
    public boolean pardonPlayer(UserInfo user, CommandSender sender, PunishmentType type,
            boolean silent) {
        if (!this.dataLoaded) {
            this.sendPrefixed(BansLang.ERROR_DATA_NOT_LOADED_ADMIN_FEEDBACK, sender);
            return false;
    ...
    public boolean pardonInet(InetAddress address, CommandSender sender, boolean silent) {
        if (!this.dataLoaded) {
            this.sendPrefixed(BansLang.ERROR_DATA_NOT_LOADED_ADMIN_FEEDBACK, sender);
            return false;
        }
    ...
    private void notifyPardon(Collection<? extends AbstractPunishment> punishments,
            CommandSender sender,
            MessageLocale feedbackLocale,
            MessageLocale broadcastLocale,
            MessageLocale notifyLocale,
    ...
    private String getKickScreenText(AbstractPunishment punishment) {
        List<String> text;
    ...
    public CompletableFuture<List<UserInfo>> lookupAltProfiles(InetAddress address) {
        return CompletableFuture.supplyAsync(() -> this.dataHandler.getProfilesByInet(address));
    }

    public void notifyAltProfiles(CommandSender sender, UserInfo profile,
            InetAddress address, List<UserInfo> alts) {
        ...
        PlaceholderContext context = PlaceholderContext.builder()
                ...
                .with(SLPlaceholders.GENERIC_ENTRY, () -> alts.stream()
                        .map(alt -> PlaceholderContext.builder().with(SLPlaceholders.GENERIC_NAME, alt::name).build()
                                .apply(

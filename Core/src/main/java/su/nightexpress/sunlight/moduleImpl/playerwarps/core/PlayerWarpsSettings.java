...
public class PlayerWarpsSettings extends AbstractConfig {
        ...
        private final ConfigProperty<Integer> warpsSaveInterval = this.addProperty(ConfigTypes.INT,
                        "Warp.Save-Interval",
                        300,
                        "Sets how often (in seconds) warp changes are saved to their disk files. Triggered on shutdown as well.",
                        "[Default is 300]");

        private final ConfigProperty<Integer> warpsIdMaxCharacters = this.addProperty(ConfigTypes.INT,
                        "Warp.Id-Max-Characters",
                        20,
                        "Sets the character limit for warp IDs upon creation.",
                        "[Default is 20]");

        private final ConfigProperty<Integer> warpsNameMaxCharacters = this.addProperty(ConfigTypes.INT,
                        "Warp.Name-Max-Characters",
                        40,
                        "Sets the character limit for warp names.",
                        "[Default is 40]");

        private final ConfigProperty<Integer> warpsDescriptionMaxCharacters = this.addProperty(ConfigTypes.INT,
                        "Warp.Description-Max-Characters",
                        80,
                        "Sets the character limit for warp descriptions.",
                        "[Default is 80]");

        private final ConfigProperty<NightItem> warpsDefaultIcon = this.addProperty(ConfigTypes.NIGHT_ITEM,
                        "Warp.Default-Icon",
                        NightItem.fromType(Material.ENDER_PEARL),
                        "Sets default icon for new warps.",
                        Placeholders.URL_WIKI_ITEMS);

        private final ConfigProperty<Set<String>> warpsWorldBlacklist = this.addProperty(
                        ConfigTypes.STRING_SET_LOWER_CASE,
                        "Warp.Creation.World_Blacklist",
                        Set.of("world_events", "custom_world"),
                        "List of worlds where warp creation is disabled.");

        private final ConfigProperty<Double> warpsCreationCost = this.addProperty(ConfigTypes.DOUBLE,
                        "Warp.Creation.Cost",
                        0D,
                        "Sets how much it costs (in Vault currency) to create a warp.",
                        "Set to '0' to disable.");

        private final ConfigProperty<RankTable> warpsAmountPerRank = this.addProperty(SLConfigTypes.RANK_TABLE,
                        "Warp.Amount_Per_Rank",
        ...
        private final ConfigProperty<Map<String, NormalCategory>> categoryMap = this.addProperty(
                        ConfigTypes.forMap(WARP_CATEGORY_CONFIG_TYPE, NormalCategory::id),
                        "Warps.Categories",
                        getDefaultCategories(),
                        "Here you can create and edit warp categories.");

        private final ConfigProperty<Boolean> featuringEnabled = this.addProperty(ConfigTypes.BOOLEAN,
                        "FeaturedWarps.Enabled",
                        true,
        ...
        private final ConfigProperty<Integer> featuredUpdateInterval = this.addProperty(ConfigTypes.INT,
                        "FeaturedWarps.Update-Interval",
                        60,
                        "Sets how often (in seconds) the featured warps cache is updated.",
                        "[Default is 60]");

        private final ConfigProperty<Map<String, FeaturedSlot>> featuringSlotMap = this.addProperty(
                        ConfigTypes.forMapWithLowerKeys(FEATURED_SLOT_CONFIG_TYPE),
                        "FeaturedWarps.Slots",
                        getDefaultPromotionSlots(),
                        "Here you can create and edit slots used for the Featued Warps feature.");

        private final ConfigProperty<Boolean> popularEnabled = this.addProperty(ConfigTypes.BOOLEAN,
                        "PopularWarps.Enabled",
                        true,
        ...
        private final ConfigProperty<Integer> popularUpdateInterval = this.addProperty(ConfigTypes.INT,
                        "PopularWarps.Update-Interval",
                        60,
        ...
        private static Map<String, FeaturedSlot> getDefaultPromotionSlots() {
                return Map.of(
                                "left",
                                new FeaturedSlot("left", CurrencyId.VAULT, 1000,
                                                TimeUnit.SECONDS.convert(7, TimeUnit.DAYS),
                                                new int[] { 1, 2 }),
                                "right",
                                new FeaturedSlot("right", CurrencyId.VAULT, 1000,
                                                TimeUnit.SECONDS.convert(7, TimeUnit.DAYS),
                                                new int[] { 6, 7 }));
        }
        ...
        private static Map<String, NormalCategory> getDefaultCategories() {
                ...
                NormalCategory category0 = new NormalCategory("default", "Default", true,
                                Lists.newList(TagWrappers.GRAY.wrap("All other warps.")),
                                NightItem.fromType(Material.COMPASS));
                NormalCategory category1 = new NormalCategory("shops", "Shops", false,
                                Lists.newList(TagWrappers.GRAY.wrap("Buy items from player shops.")),
                                NightItem.fromType(Material.EMERALD));
                NormalCategory category2 = new NormalCategory("farms", "Farms", false,
                                Lists.newList(TagWrappers.GRAY.wrap("Farm mobs and items.")),
                                NightItem.fromType(Material.CARROT));

...
public class ChatDefaults {
        ...
        public static final String DEFAULT_CHANNEL_FORMAT = GRAY.wrap("[" + CHANNEL_NAME + "]") + " "
                        + SLPlaceholders.GENERIC_FORMAT;
        public static final String DEFAULT_LOCAL_CHANNEL_ID = "local";

        public static final String DEFAULT_USER_FORMAT = CommonPlaceholders.PLAYER_NAME + ": "
                        + SLPlaceholders.GENERIC_FORMAT;
        ...
        public static Map<String, FormatComponent> getDefaultFormatComponents() {
                ...
                map.put("player_info", new FormatComponent(
                                SHOW_TEXT
                                                .with(GRAY.wrap("Player: " + LIGHT_PURPLE.wrap(PLAYER_NAME) + BR
                                                                + "Nickname: "
                                                                + LIGHT_PURPLE.wrap(PLAYER_DISPLAY_NAME) + BR + BR
                                                                + GRAY.wrap("(Click to message)")))
                                                .wrap(SUGGEST_COMMAND
                                                                .with("/" + DEFAULT_PM_ALIAS + " " + PLAYER_NAME + " ")
                                                                .wrap(PLAYER_DISPLAY_NAME))));

                map.put("rank_owner", new FormatComponent(
                                SHOW_TEXT.with(GRAY.wrap("This player is the server " + SOFT_YELLOW.wrap("Owner")))
                                                .wrap(PLAYER_PREFIX)));

                map.put("rank_member", new FormatComponent(
                                SHOW_TEXT.with(GRAY.wrap(
                                                "Type " + SOFT_GREEN.wrap("/ranks") + " to view all special ranks."))
                                                .wrap(RUN_COMMAND.with("/ranks").wrap(PLAYER_PREFIX))));
        ...
        public static Collection<String> getDefaultWordFilterRules(Locale locale) {
        ...
        public static Map<SpyType, String> getDefaultSpyFormat() {
                ...
                map.put(SpyType.SOCIAL,
                                SOFT_RED.wrap("[SocialSpy]") + " "
                                                + GRAY.wrap(PLAYER_NAME + " ➡ " + SLPlaceholders.GENERIC_TARGET + ":")
                                                + " "
                                                + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE));
                map.put(SpyType.COMMAND,
                                SOFT_RED.wrap("[CommandSpy]") + " " + GRAY.wrap(PLAYER_NAME + " executed a command:")
                                                + " " + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE));
                map.put(SpyType.CHAT,
                                SOFT_RED.wrap("[ChatSpy]") + " "
                                                + GRAY.wrap("[" + CHANNEL_NAME + "]" + " " + PLAYER_NAME + ":") + " "
                                                + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE));
                return map;
        }
}

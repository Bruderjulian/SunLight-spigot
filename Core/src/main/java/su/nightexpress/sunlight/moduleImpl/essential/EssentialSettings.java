...
import static su.nightexpress.sunlight.moduleImpl.essential.EssentialPlaceholders.*;
...
public class EssentialSettings extends AbstractConfig {
        ...
        public final ConfigProperty<Integer> disposalSize = this.addProperty(ConfigTypes.INT, "Disposal.InventorySize",
                        36);
        ...
        public final ConfigProperty<List<String>> nearFormatNormal = this.addProperty(ConfigTypes.STRING_LIST,
                        ...
                        Lists.newList(
                                        ...
                                        GRAY.wrap("There are " + SOFT_YELLOW.and(UNDERLINED).wrap(GENERIC_AMOUNT)
                                                        + " players around you:"),
                                        " ",
                                        GENERIC_ENTRY,
                                        DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(32))),
                        "[*] Text Formations Guide: " + URL_WIKI_TEXT,
                        "[*] Placeholders: '%s', '%s', 'PlaceholderAPI'".formatted(GENERIC_RADIUS, GENERIC_AMOUNT));
        ...
        public final ConfigProperty<String> nearEntryFormat = this.addProperty(ConfigTypes.STRING, "Near.Format.Entry",
                        HEAD_HAT.apply(PLAYER_NAME) + " " + GRAY.wrap(PLAYER_PREFIX + SUGGEST_COMMAND
                                        .with("/ptp request " +
                                                        PLAYER_NAME)
                                        .wrap(SHOW_TEXT.with(GRAY.wrap("Click to send teleport request.")).wrap(
                                                        WHITE.wrap(
                                                                        PLAYER_DISPLAY_NAME)))
                                        + PLAYER_SUFFIX + " " + DARK_GRAY.wrap("(" + GRAY.wrap(GENERIC_DISTANCE +
                                                        " blocks " + GENERIC_DIRECTION) + ")")),
                        "Player entry format.",
                        "[*] Text Formations Guide: " + URL_WIKI_TEXT,
                        "[*] Placeholders: '%s', '%s', 'PlaceholderAPI'".formatted(GENERIC_DISTANCE,
                                        GENERIC_DIRECTION));

        public final ConfigProperty<Boolean> nearUseArrows = this.addProperty(ConfigTypes.BOOLEAN,
                        "Near.Format.Use_Arrows",
                        true,
                        "If enabled, the '%s' placeholder in the entry format will display direction arrows instead of localized direction names."
                                        .formatted(GENERIC_DIRECTION));

        public final ConfigProperty<Map<String, String>> nearDirectionArrows = this.addProperty(
                        ConfigTypes.forMapWithLowerKeys(ConfigTypes.STRING),
                        ...
                        Map.of(
                                        ...
                                        Direction.HERE.name(), "•"),
                        "Direction arrow glyphs used by the '%s' placeholder when '%s' is enabled.".formatted(
                                        GENERIC_DIRECTION,
                                        "Near.Format.Use_Arrows"));
        ...
        public final ConfigProperty<List<String>> playerInfoFormat = this.addProperty(ConfigTypes.STRING_LIST,
                        "PlayerInfo.Format",
                        Lists.newList(
                                        DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(12)) + " "
                                                        + GRAY.wrap(SOFT_YELLOW.wrap(PLAYER_NAME) +
                                                                        " Info")
                                                        + " " + DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(12)),
                                        " ",
                                        DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("server_list/ping_5")
                                                        + GRAY.wrap(" Status: ") + GREEN.wrap(
                                                                        PLAYER_STATUS),
                                        DARK_GRAY.wrap("» ") + SPRITE_ITEMS.apply("item/clock_12")
                                                        + GRAY.wrap(" Since: ") + WHITE.wrap(
                                                                        PLAYER_LAST_SEEN),
                                        " ",
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("UUID: ")
                                                        + COPY_TO_CLIPBOARD.with(PLAYER_UUID).wrap(SHOW_TEXT.with(GRAY
                                                                        .wrap("Click to copy!"))
                                                                        .wrap(WHITE.wrap(PLAYER_UUID))),
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("Name: ")
                                                        + COPY_TO_CLIPBOARD.with(PLAYER_NAME).wrap(SHOW_TEXT.with(GRAY
                                                                        .wrap("Click to copy!"))
                                                                        .wrap(WHITE.wrap(PLAYER_NAME))),
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("Nickname: ")
                                                        + COPY_TO_CLIPBOARD.with(PLAYER_DISPLAY_NAME).wrap(SHOW_TEXT
                                                                        .with(GRAY.wrap("Click to copy!"))
                                                                        .wrap(WHITE.wrap(PLAYER_DISPLAY_NAME))),
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("IP: ")
                                                        + COPY_TO_CLIPBOARD.with(PLAYER_INET_ADDRESS).wrap(SHOW_TEXT
                                                                        .with(
                                                                                        GRAY.wrap("Click to copy!"))
                                                                        .wrap(WHITE.wrap(PLAYER_INET_ADDRESS))),
                                        " ",
                                        DARK_GRAY.wrap("» ") + SPRITE_ITEMS.apply("item/compass_15")
                                                        + GRAY.wrap(" Location: ")
                                                        + WHITE.wrap(RED
                                                                        .wrap(LOCATION_X) + " " + GREEN.wrap(LOCATION_Y)
                                                                        + " " + BLUE.wrap(LOCATION_Z)
                                                                        + " @ " +
                                                                        LOCATION_WORLD)
                                                        + " "
                                                        + RUN_COMMAND.with("/tppos " + LOCATION_X + " " + LOCATION_Y
                                                                        + " " + LOCATION_Z +
                                                                        " " + LOCATION_WORLD)
                                                                        .wrap(SOFT_YELLOW.wrap("[Teleport]")),

                                        DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.NETHER_STAR)
                                                        + GRAY.wrap(" Game Mode: ")
                                                        + WHITE.wrap(
                                                                        PLAYER_GAME_MODE),
                                        DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.FEATHER)
                                                        + GRAY.wrap(" Can Fly: ")
                                                        + PLAYER_CAN_FLY,
                                        DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.EXPERIENCE_BOTTLE)
                                                        + GRAY.wrap(
                                                                        " XP Level: ")
                                                        + PLAYER_LEVEL,
                                        DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("hud/food_full")
                                                        + GRAY.wrap(" Hunger / Saturation: ")
                                                        + WHITE.wrap(
                                                                        PLAYER_FOOD_LEVEL)
                                                        + GRAY.wrap(" / ") + SOFT_ORANGE.wrap(PLAYER_SATURATION),
                                        DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("hud/heart/full")
                                                        + GRAY.wrap(" Health / Max. Health: ")
                                                        + WHITE
                                                                        .wrap(PLAYER_HEALTH + "❤")
                                                        + GRAY.wrap(" / ") + SOFT_RED.wrap(PLAYER_MAX_HEALTH + "❤"),
                                        DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("mob_effect/invisibility")
                                                        + GRAY.wrap(" Vanished: ")
                                                        + WHITE.wrap(
                                                                        PLAYER_VANISHED),
                                        " ",
        ...
        public final ConfigProperty<List<String>> staffFormat = this.addProperty(ConfigTypes.STRING_LIST,
                        ...
                        List.of(
                                        DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(32)),
                                        GRAY.wrap("There are " + SOFT_RED.and(UNDERLINED).wrap(GENERIC_AMOUNT)
                                                        + " staff players online:"),
                                        " ",
                                        GENERIC_ENTRY,
                                        DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(32))),
                        "[*] Text Formations Guide: " + URL_WIKI_TEXT,
        ...
        public final ConfigProperty<String> staffEntryFormat = this.addProperty(ConfigTypes.STRING,
                        "Staff.Format.Entry",
                        HEAD_HAT.apply(PLAYER_NAME) + " "
                                        + GRAY.wrap(PLAYER_PREFIX + SUGGEST_COMMAND.with("/msg " + PLAYER_NAME + " ")
                                                        .wrap(SHOW_TEXT.with(
                                                                        GRAY.wrap("Click to write private message."))
                                                                        .wrap(WHITE.wrap(
                                                                                        PLAYER_DISPLAY_NAME)))
                                                        + PLAYER_SUFFIX),
                        "[*] Text Formations Guide: " + URL_WIKI_TEXT,
                        "[*] Placeholders: '%s', '%s', 'PlaceholderAPI'".formatted(PLAYER_PREFIX, PLAYER_SUFFIX));
        ...
        public final ConfigProperty<List<String>> timeDisplayFormat = this.addProperty(ConfigTypes.STRING_LIST,
                        "Time.Show.Format", List.of(
                                        DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(12)) + " " + GRAY.wrap("Time Info")
                                                        + " "
                                                        + DARK_GRAY.and(
                                                                        STRIKETHROUGH).wrap("-".repeat(12)),
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("World: " + WHITE.wrap(GENERIC_WORLD)),
                                        DARK_GRAY.wrap("» ") + GRAY
                                                        .wrap("World Time: " + SOFT_YELLOW.wrap(GENERIC_TIME) + " ("
                                                                        + WHITE.wrap(
                                                                                        GENERIC_TICKS + " ticks")
                                                                        + ")"),
                                        DARK_GRAY.wrap("» ")
        ...
        public boolean isInvulnerabilityAllowedInThisWorld(World world) {
                return !this.invulnerabilityDisabledWorlds.get().contains(Utils.lowercase(world.getName()));
        }

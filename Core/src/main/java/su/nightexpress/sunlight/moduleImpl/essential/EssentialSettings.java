...
public class EssentialSettings extends AbstractConfig {
    ...
    public final ConfigProperty<List<String>> nearFormatOthers = this.addProperty(ConfigTypes.STRING_LIST,
            ...
            Lists.newList(
                    DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(32)),
                    GRAY.wrap("There are " + SOFT_YELLOW.and(UNDERLINED).wrap(GENERIC_AMOUNT) + " players around "
                            + WHITE.wrap(
                                    PLAYER_DISPLAY_NAME)
                            + ":"),
                    " ",
    ...
    public final ConfigProperty<String> nearEntryFormat = this.addProperty(ConfigTypes.STRING, "Near.Format.Entry",
            HEAD_HAT.apply(PLAYER_NAME) + " " + GRAY.wrap(PLAYER_PREFIX + SUGGEST_COMMAND.with("/ptp request " +
                    PLAYER_NAME).wrap(SHOW_TEXT.with(GRAY.wrap("Click to send teleport request.")).wrap(
                            WHITE.wrap(
                                    PLAYER_DISPLAY_NAME)))
            ...
            "Player entry format.",
            "[*] Text Formations Guide: " + URL_WIKI_TEXT,
            "[*] Placeholders: '%s', '%s', '%s'".formatted(GENERIC_DISTANCE, GENERIC_DIRECTION,
    ...
    public final ConfigProperty<List<String>> playerInfoFormat = this.addProperty(ConfigTypes.STRING_LIST,
            ...
            Lists.newList(
                    ...
                    DARK_GRAY.wrap("» ") + GRAY.wrap("IP: ")
                            + COPY_TO_CLIPBOARD.with(PLAYER_INET_ADDRESS).wrap(SHOW_TEXT.with(
                    ...
                    DARK_GRAY.wrap("» ") + SPRITE_ITEMS.apply("item/compass_15") + GRAY.wrap(" Location: ")
                            + WHITE.wrap(RED
                                    ...
                                    + " @ " +
                                    LOCATION_WORLD)
                            + " " + RUN_COMMAND.with("/tppos " + LOCATION_X + " " + LOCATION_Y + " " + LOCATION_Z +
                                    " " + LOCATION_WORLD).wrap(SOFT_YELLOW.wrap("[Teleport]")),

                    DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.NETHER_STAR) + GRAY.wrap(" Game Mode: ")
                            + WHITE.wrap(
                                    PLAYER_GAME_MODE),
                    DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.FEATHER) + GRAY.wrap(" Can Fly: ")
                            + PLAYER_CAN_FLY,
                    DARK_GRAY.wrap("» ") + SPRITE_ITEM.apply(Material.EXPERIENCE_BOTTLE) + GRAY.wrap(
                            " XP Level: ") + PLAYER_LEVEL,
                    DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("hud/food_full") + GRAY.wrap(" Hunger / Saturation: ")
                            + WHITE.wrap(
                                    PLAYER_FOOD_LEVEL)
                            + GRAY.wrap(" / ") + SOFT_ORANGE.wrap(PLAYER_SATURATION),
                    DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("hud/heart/full") + GRAY.wrap(" Health / Max. Health: ")
                            + WHITE
                                    .wrap(PLAYER_HEALTH + "❤")
                            + GRAY.wrap(" / ") + SOFT_RED.wrap(PLAYER_MAX_HEALTH + "❤"),
                    DARK_GRAY.wrap("» ") + SPRITE_GUI.apply("mob_effect/invisibility") + GRAY.wrap(" Vanished: ")
                            + WHITE.wrap(
                                    PLAYER_VANISHED),
                    " ",
                    DARK_GRAY.and(STRIKETHROUGH).wrap("-".repeat(32))),
    ...
    public final ConfigProperty<List<String>> timeDisplayFormat = this.addProperty(ConfigTypes.STRING_LIST,
            "Time.Show.Format", List.of(
                    ...
                    DARK_GRAY.wrap("» ") + GRAY.wrap("World Time: " + SOFT_YELLOW.wrap(GENERIC_TIME) + " ("
                            + WHITE.wrap(
                                    GENERIC_TICKS + " ticks")
                            + ")"),

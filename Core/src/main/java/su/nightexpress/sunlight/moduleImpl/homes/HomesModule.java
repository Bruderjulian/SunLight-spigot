package su.nightexpress.sunlight.moduleImpl.homes;
...
import su.nightexpress.sunlight.moduleImpl.homes.config.HomesSettings;
import su.nightexpress.sunlight.moduleImpl.homes.data.HomeDataManager;
import su.nightexpress.sunlight.moduleImpl.homes.dialog.HomeDialogKeys;
import su.nightexpress.sunlight.moduleImpl.homes.dialog.impl.HomeDeleteDialog;
import su.nightexpress.sunlight.moduleImpl.homes.dialog.impl.HomeInvitePlayerDialog;
import su.nightexpress.sunlight.moduleImpl.homes.dialog.impl.HomeNameDialog;
import su.nightexpress.sunlight.moduleImpl.homes.event.PlayerHomeCreateEvent;
import su.nightexpress.sunlight.moduleImpl.homes.event.PlayerHomeRemoveEvent;
import su.nightexpress.sunlight.moduleImpl.homes.event.PlayerHomeTeleportEvent;
import su.nightexpress.sunlight.moduleImpl.homes.impl.Home;
import su.nightexpress.sunlight.moduleImpl.homes.listener.HomeListener;
import su.nightexpress.sunlight.moduleImpl.homes.menu.HomeSettingsMenu;
import su.nightexpress.sunlight.moduleImpl.homes.menu.HomesMenu;
import su.nightexpress.sunlight.moduleImpl.homes.menu.IconSelectionMenu;
import su.nightexpress.sunlight.moduleImpl.homes.menu.InvitedPlayersMenu;
import su.nightexpress.sunlight.moduleImpl.homes.repository.GlobalHomeRepository;
import su.nightexpress.sunlight.moduleImpl.homes.repository.UserHomeRepository;
...
public class HomesModule extends Module {
    ...
    public int getMaxHomesValue(Player player) {
        return this.settings.getHomesByRankAmount().getGreatestOrNegative(player).intValue();
    }

    private boolean checkDataLoaded(CommandSender sender) {
        if (!this.loaded) {
    ...
    public void handleBedInteract(PlayerInteractEvent event) {
        if (!this.settings.isBedModeEnabled())
            return;
        ...
        if (block == null || !(block.getBlockData() instanceof Bed))
            return;
        if (!(block.getState() instanceof Colorable colorable))
            return;
        ...
        if (currentHome == null || player.isSneaking()) {
            ...
            if (!this.setHome(player, homeId, location, false))
                return;
    ...
    public boolean checkLocation(Player player, Location location, boolean notify) {
        if (this.settings.isCheckBuildAccess() && !player.hasPermission(HomesPerms.BYPASS_CREATION_PROTECTION)) {
            ...
            if (event.isCancelled()) {
                if (notify)
                    this.sendPrefixed(HomesLang.HOME_SET_ERROR_PROTECTION, player);
                return false;
            }
        }
    ...
    public boolean setHome(Player player, String name, boolean force) {
    ...
    public boolean setHome(Player player, String aname, Location location, boolean force) {
        if (!this.checkDataLoaded(player))
            return false;
        ...
        if (!force) {
            ...
            if (currentHome == null) {
                chargeCreation = creationCost > 0D && !EconomyUtils.hasBypass(player, HomesPerms.BYPASS_COST)
                        && EconomyUtils
                                .hasCurrency();
    ...
    public boolean teleportToHome(Player player, Home home) {
        ...
        boolean charge = cost > 0D && !EconomyUtils.hasBypass(player, HomesPerms.BYPASS_COST)
    ...
    public boolean visitHome(Player player, Home home) {
    ...
    public boolean inviteToHome(Player player, Home home, UserInfo profile) {

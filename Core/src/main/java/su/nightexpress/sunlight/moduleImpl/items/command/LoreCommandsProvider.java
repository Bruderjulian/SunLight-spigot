...
public class LoreCommandsProvider extends CommandProvider {
    ...
    public LoreCommandsProvider(SunLightPlugin plugin, ItemsModule module) {
        super(plugin);
        this.module = module;
    }
    ...
    private boolean addOrInsertLoreLine(CommandContext context, ParsedArguments arguments) {
    ...
    private boolean removeLoreLine(CommandContext context, ParsedArguments arguments) {
        return CommandArguments.handleItemInHandOrError(context, (player, itemStack) -> {
            int index = arguments.getInt(CommandArguments.POSITION);

            List<String> lore = ItemUtil.getLoreSerialized(itemStack);
            if (lore.isEmpty()) {
                this.module.sendPrefixed(ItemsLang.ERROR_ITEM_HAS_NO_LORE, context.getSender(),
                        builder -> builder.with(GENERIC_ITEM, () -> ItemUtil.getNameSerialized(itemStack)));
                return false;
            }
    ...
    private boolean clearLore(CommandContext context, ParsedArguments arguments) {
        return CommandArguments.handleItemInHandOrError(context, (player, itemStack) -> {
            List<String> lore = ItemUtil.getLoreSerialized(itemStack);
            if (lore.isEmpty()) {

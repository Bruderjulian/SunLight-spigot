...
public class HealthCommandProvider extends AbstractCommandProvider {
        ...
        public HealthCommandProvider(SunLightPlugin plugin, EssentialModule module, EssentialSettings settings,
        ...
        public void registerDefaults() {
                this.registerLiteral(COMMAND_ADD, false, new String[] { "addhealth" },
                                builder -> this.builderMode(builder, ModifyMode.ADD));
                this.registerLiteral(COMMAND_SET, false, new String[] { "sethealth" },
                                builder -> this.builderMode(builder, ModifyMode.SET));
                this.registerLiteral(COMMAND_REMOVE, false, new String[] { "removehealth" },
                ...
                this.registerLiteral(COMMAND_RESTORE, true, new String[] { "heal" }, builder -> builder
                                ...
                                .withArguments(Arguments.playerName(CommandArguments.PLAYER).permission(PERM_OTHERS)
                                                .optional())
                ...
                builder
                                ...
                                .withArguments(
                                                Arguments.decimal(CommandArguments.AMOUNT, 0)
                                                                ...
                                                                .suggestions((reader, tabContext) -> IntStream
                                                                                .range(0, 21).boxed()
                                                                                .map(String::valueOf)
                                                                                .toList()),
                                                Arguments.playerName(CommandArguments.PLAYER).permission(PERM_OTHERS)
                                                                .optional())
                                .withFlags(CommandArguments.FLAG_SILENT)
                                .executes((context, arguments) -> this.changeHealth(context, arguments, mode));
        }

        private boolean changeHealth(CommandContext context, ParsedArguments arguments, ModifyMode mode) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        if (target.isDead()) {
                                                context.send(context.getSender() == target ? MESSAGE_DEAD_NOTIFY
                                                                : MESSAGE_DEAD_FEEDBACK,
                                                                replacer -> replacer.replace(forPlayer(target)));
                                                return;
                                        }
                                        ...
                                        if (target != context.getSender()) {
                                                this.module.sendPrefixed(feedbackMessage, context.getSender(),
                                                                builder -> builder
                                                                                .with(GENERIC_OLD_VALUE,
                                                                                                () -> NumberUtil.format(
                                                                                                                oldHealth))
                                                                                .with(GENERIC_NEW_VALUE,
                                                                                                () -> NumberUtil.format(
                                                                                                                target.getHealth()))
                                                                                .with(GENERIC_AMOUNT,
                                                                                                () -> NumberUtil.format(
                                                                                                                amount))
                                                                                .with(GENERIC_CURRENT, () -> NumberUtil
                                                                                                .format(target.getHealth())) // old
                                                                                .with(GENERIC_MAX, () -> NumberUtil
                                                                                                .format(maxHealth)) // old
                                        ...
                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                                                this.module.sendPrefixed(notifyMessage, target, builder -> builder
                                                                .with(GENERIC_OLD_VALUE,
                                                                                () -> NumberUtil.format(oldHealth))
                                                                .with(GENERIC_NEW_VALUE,
                                                                                () -> NumberUtil.format(
                                                                                                target.getHealth()))
                                                                .with(GENERIC_AMOUNT, () -> NumberUtil.format(amount))
                                                                .with(GENERIC_CURRENT,
                                                                                () -> NumberUtil.format(
                                                                                                target.getHealth())) // old
        ...
        private boolean restoreHealth(CommandContext context, ParsedArguments arguments) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        ...
                                        if (target.isDead()) {
                                                ...
                                                double maxHealth = EntityUtil.getAttributeValue(target,
                                                                Attribute.MAX_HEALTH);

                                                target.setHealth(maxHealth);
                                                if (this.settings.healthClearEffects.get()) {
                                                        target.getActivePotionEffects().forEach(effect -> target
                                                                        .removePotionEffect(effect.getType()));
                                                }
                                        }

                                        if (target != context.getSender()) {
                                                this.module.sendPrefixed(feedbackLocale, context.getSender(),
                                                                builder -> builder.with(CommonPlaceholders.PLAYER
                                                                                .resolver(target)));
                                        }
                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {

...
import su.nightexpress.sunlight.moduleImpl.playerwarps.PlayerWarpRepository;
...
import static su.nightexpress.sunlight.moduleImpl.warps.WarpsPlaceholders.*;

public class PlayerWarpsListMenu extends AbstractObjectMenu<WarpsListData> implements LangContainer {

        private static final IconLocale ICON_WARP_DEFAULT = LangEntry
                        .iconBuilder("PlayerWarps.UI.WarpsMenu.WarpDefault")
                        .rawName(WARP_NAME)
                        .rawLore(
                                        DARK_GRAY.wrap("» ") + GRAY.wrap("Owner: "
                                                        + WHITE.wrap(PlayerWarpsPlaceholders.WARP_OWNER_NAME)),
                                        DARK_GRAY.wrap("» ") + GRAY.wrap(
                                                        "Visits: " + WHITE.wrap(PlayerWarpsPlaceholders.WARP_VISITS)),
        ...
        private static final IconLocale ICON_WARP_MODERATE = LangEntry
                        .iconBuilder("PlayerWarps.UI.WarpsMenu.WarpModerate")
                        .rawName(WARP_NAME)
                        .rawLore(
        ...
        protected String getRawTitle(ViewerContext context) {
                return PlaceholderContext.builder()
                                .with(GENERIC_CATEGORY, () -> this.getObject(context).category().name())
                ...
                this.addDefaultButton("search", MenuItem.builder()
                                ...
                                .state(ItemState.builder("with_input")
                                                ...
                                                .displayModifier((context, item) -> item.replace(
                                                                builder -> builder.with(GENERIC_INPUT,
                ...
                this.addDefaultButton("sorting", MenuItem.builder()
                                .defaultState(ItemState.defaultBuilder()
                                                ...
                                                .displayModifier((context, item) -> item.replace(builder -> builder
                                                                .with(GENERIC_TYPE,
                                                                                () -> this.getObject(context).sortType()
                                                                                                .localized())))
        ...
        protected void onLoad(FileConfig config) {
                int[] warpSlots = ConfigProperty
                                .of(ConfigTypes.INT_ARRAY, "Warps.Slots", IntStream.range(9, 45).toArray())
                                ...
                                .itemProvider((context, warp) -> {
                                        ...
                                        return warp.getIcon()
                                                        ...
                                                        .localized(warp.isOwner(player) ? ICON_WARP_OWN
                                                                        : (warp.canEdit(player) ? ICON_WARP_MODERATE
                                                                                        : ICON_WARP_DEFAULT))

...
public class NickCommandsProvider extends AbstractCommandProvider {
        ...
        private static final TextLocale COMMAND_NICK_ROOT_DESC = LangEntry.builder("Command.Nick.Root.Desc")
                        .text("Nick commands.");
        private static final TextLocale COMMAND_NICK_CLEAR_DESC = LangEntry.builder("Command.Nick.Clear.Desc")
                        .text("Remove custom name.");
        private static final TextLocale COMMAND_NICK_SET_DESC = LangEntry.builder("Command.Nick.Set.Desc")
                        .text("Set player's custom name.");
        private static final TextLocale COMMAND_NICK_CHANGE_DESC = LangEntry.builder("Command.Nick.Change.Desc")
                        .text("Set custom name.");

        private static final MessageLocale COMMAND_NICK_CLEAR_TARGET = LangEntry.builder("Command.Nick.Clear.Target")
                        .chatMessage(
                                        GRAY.wrap("Removed " + SOFT_YELLOW.wrap(PLAYER_DISPLAY_NAME)
                                                        + "'s custom name."));

        private static final MessageLocale COMMAND_NICK_CLEAR_NOTIFY = LangEntry.builder("Command.Nick.Clear.Notify")
                        .chatMessage(
                                        GRAY.wrap("You custom name has been removed."));

        private static final MessageLocale COMMAND_NICK_SET_TARGET = LangEntry.builder("Command.Nick.Set.Target")
                        .chatMessage(
                                        GRAY.wrap("Set " + SOFT_YELLOW.wrap(PLAYER_NAME) + "'s custom name to "
                                                        + SOFT_YELLOW.wrap(GENERIC_NAME) + "."));

        private static final MessageLocale COMMAND_NICK_SET_NOTIFY = LangEntry.builder("Command.Nick.Set.Notify")
                        .chatMessage(
                                        GRAY.wrap("You got a new custom name: " + SOFT_YELLOW.wrap(GENERIC_NAME)
                                                        + "."));

        private static final MessageLocale COMMAND_NICK_CHANGE_DONE = LangEntry.builder("Command.Nick.Change.Done")
                        .chatMessage(
                                        GRAY.wrap("You changed your custom name to " + SOFT_YELLOW.wrap(GENERIC_NAME)
                                                        + "."));

        private static final MessageLocale COMMAND_NICK_CHANGE_ERROR_BAD_WORDS = LangEntry
                        .builder("Command.Nick.Error.BadWords").chatMessage(
                                        SOFT_RED.wrap("This name is not allowed."));

        private static final MessageLocale COMMAND_NICK_CHANGE_ERROR_REGEX = LangEntry
                        .builder("Command.Nick.Error.Regex")
                        .chatMessage(
                                        SOFT_RED.wrap("Name contains forbidden characters."));

        private static final MessageLocale COMMAND_NICK_CHANGE_ERROR_TOO_LONG = LangEntry
                        .builder("Command.Nick.Error.TooLong").chatMessage(
                                        GRAY.wrap("Name can't be no longer than " + SOFT_RED.wrap(GENERIC_AMOUNT)
                                                        + " characters."));

        private static final MessageLocale COMMAND_NICK_CHANGE_ERROR_TOO_SHORT = LangEntry
                        .builder("Command.Nick.Error.TooShort").chatMessage(
                                        SOFT_RED.wrap("Name can't be shorted than " + SOFT_RED.wrap(GENERIC_AMOUNT)
                                                        + " characters."));
        ...
        public NickCommandsProvider(SunLightPlugin plugin, EssentialModule module, EssentialSettings settings,
        ...
        private boolean changeNick(CommandContext context, ParsedArguments arguments) {
                ...
                if (!player.hasPermission(NICK_BYPASS_LENGTH)) {
                        if (raw.length() < this.settings.nickMinLength.get()) {
                                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_SHORT, context.getSender(),
                                                replacer -> replacer
                                                                .with(SLPlaceholders.GENERIC_AMOUNT, () -> String
                                                                                .valueOf(this.settings.nickMinLength
                                                                                                .get())));
                                return false;
                        }
                        if (raw.length() > this.settings.nickMaxLength.get()) {
                                this.module.sendPrefixed(COMMAND_NICK_CHANGE_ERROR_TOO_LONG, context.getSender(),
                                                replacer -> replacer
                                                                .with(SLPlaceholders.GENERIC_AMOUNT, () -> String
                                                                                .valueOf(this.settings.nickMaxLength
                                                                                                .get())));
                                return false;
                        }
                }
                ...
                if (!player.hasPermission(NICK_BYPASS_WORDS)) {
                        if (this.settings.nickBannedWords.get().stream()
                ...
                this.module.sendPrefixed(COMMAND_NICK_CHANGE_DONE, context.getSender(),
                ...
                return this.loadPlayerOrSenderWithDataAndRunInMainThread(context, arguments, this.module,
                                ...
                                (user, target) -> {
                                        ...
                                        if (context.getSender() != target) {
                                                this.module.sendPrefixed(COMMAND_NICK_CLEAR_TARGET, context.getSender(),
                ...
                return this.loadPlayerWithDataAndRunInMainThread(context, arguments, this.module, this.userManager,
                                (user, target) -> {
                                        ...
                                        if (context.getSender() != user.player().orElse(null)) {
                                                ...
                                                this.module.sendPrefixed(COMMAND_NICK_SET_TARGET, context.getSender(),
                                                                replacer -> replacer
                                        ...
                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                                                String finalNick = nick;
                                                this.module.sendPrefixed(COMMAND_NICK_SET_NOTIFY, target,

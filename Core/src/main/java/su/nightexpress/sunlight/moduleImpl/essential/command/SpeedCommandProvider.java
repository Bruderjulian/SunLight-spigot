...
public class SpeedCommandProvider extends CommandProvider {
        ...
        public SpeedCommandProvider(SunLightPlugin plugin, EssentialModule module, UserManager userManager) {
                super(plugin);
                this.module = module;
        ...
        private boolean changeSpeed(CommandContext context, ParsedArguments arguments) {
                return this.loadPlayerOrSenderAndRunInMainThread(context, arguments, this.module, this.userManager,
                                target -> {
                                        ...
                                        if (context.getSender() != target) {
                                                this.module.sendPrefixed(MESSAGE_SET_SPEED_FEEDBACK,
                                                                context.getSender(), replacer -> replacer
                                                                                .with(CommonPlaceholders.PLAYER
                                                                                                .resolver(target))
                                                                                .with(GENERIC_AMOUNT, () -> String
                                                                                                .valueOf(speed)));
                                        }

                                        if (!context.hasFlag(CommandArguments.FLAG_SILENT)) {
                                                this.module.sendPrefixed(MESSAGE_SET_SPEED_NOTIFY, target,
                                                                replacer -> replacer
                                                                                .with(GENERIC_AMOUNT, () -> String
                                                                                                .valueOf(speed)));
                                        }
